<form action="{{ route('advices.update', [currentAccount(), $advice->uuid]) }}" method="POST" id="advice-form"
    data-form="ajax-form" data-datatable="#advices-table" data-modal="#ajax_model">
    @csrf
    @method('put')
    <div class="modal-header align-center">
        <div class="nk-file-title">
            ...
            <div class="nk-file-name">
                <div class="nk-file-name-text"><span class="title">Update Advice</span></div>
            </div>
        </div>
    ...
    <div class="modal-body">
        <div class="form-group">
            <label for="name">Advices Title</label>
            <input type="text" value="{{ $advice->title }}" class="form-control" name="advice_title"
                placeholder="Enter advice Title" required>
        </div>
        <div class="form-group">
            <label for="name">Secondry Title</label>
            <input type="text" value="{{ $advice->secondary_title }}" class="form-control" name="secondary_title" placeholder="Enter advice Secondary Title" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="default-06">Select Categories</label>
            <div id="sortable">
                @if(count($adviceCategories))
                @foreach ($adviceCategories as $adviceCategory)
                                ...
                                <div class="form-control-wrap ">
                                    ...
                                    <select class="js-example-basic-multiple" name="categories[]" data-search="off">
                                        ...
                                        @foreach ($categories as $category)
                                            <option @if ($adviceCategory == $category->id) selected @endif value="{{ $category->id }}">
                                                {{ $category->id }} {{ $category->secondary_title }}</option>
                                        @endforeach
                                    </select>
