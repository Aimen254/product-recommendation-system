<form action="{{ route('products.update', [currentAccount(), $product->uuid]) }}" method="POST" id="product-form" enctype="multipart/form-data" data-form="ajax-form" data-datatable="#products-table" data-modal="#ajax_model">
    @csrf
    @method('put')
    <div class="modal-header align-center">
        <div class="nk-file-title">
            ...
            <div class="nk-file-name">
                <div class="nk-file-name-text"><span class="title">Edit Product</span></div>
            </div>
        </div>
    ...
    <div class="modal-body">
        <div class="form-group text-center">
            ...
            <div class="form-control-wrap">
                <div class="profile-picture-upload">
                    {{-- image testing --}}
                    <img src="{{ $product->image ? asset('storage/images/' . $product->image) : asset('images/default.png') }}" alt="" class="imagePreviewLogo imagePreview d-block mx-auto mt-1 mb-4">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="{{ $product->title }}" name="product_title" placeholder="Enter Product Title">
        </div>
        <div class="form-group">
            <label for="name">Product Description</label>
            <textarea class="form-control summernote" name="product_description" aria-label="With textarea" spellcheck="false">
            {{ $product->description }}</textarea>
            <span class="text" id="charNum">Only 142 characters will be visible on frontend</span>
        </div>
        <div class="form-group">
            <label for="name">Product url</label>
            <input type="text" class="form-control" value="{{ $product->url }}" name="product_url" placeholder="Enter Product url">
        </div>

        <!-- Update the Product Setup Field Names with their values -->
        ...
        <div class="form-group">
            ...
            <select class="form-select" multiple="multiple" data-search="on" name="{{ $group['fieldName'] }}[]" data-placeholder="Select value">
                ...
                <option value="{{ $valueId }}" @foreach($productSelectedValues as $selectedValue) {{ $selectedValue->value_id == $valueId ? 'selected' : '' }} @endforeach>{{ $value }}</option>
                @endforeach
            </select>
        </div>
        @endforeach
